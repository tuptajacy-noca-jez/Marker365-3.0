       ...
       public Cart(string User)
        {
            ...
            SqlCommand cmd = new SqlCommand("select [products].*,[cartPosition].[quantity] from [cartPosition] INNER JOIN [products] ON[cartPosition].IdProduct =[products].Id WHERE[cartPosition].IdCard = 'sr306';", sql); // zamiast sr306 ma być currUser - sr306 tylko do testów
            sql.Open();
